			...
			if (preg_match('/^[a-zA-Z0-9]+$/', $_POST["loginUser"]) && 
				...
				if($answer["user"] == $_POST["loginUser"] && $answer["password"] == $encryptpass){
					...
					if($answer["status"] == 1){
						...
						echo '<br><div style="font-family: NRT;" class="alert alert-danger">ئەم بەکارهێنەرە چاڵاک نیە</div>';
					
					}

				}else{

					echo '<br><div style="font-family: NRT;" class="alert alert-danger">ناوی بەکارهێنەر یان ووشەی تێپەر هەڵەیە</div>';
		...
		if (isset($_POST["newUser"])) {
			...
			if (preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["newName"]) &&
				...
				if ($answer == 'ok') {
						...
						swal({
							type: "success",
							title: "بەکارهێنەرەکە بە سەرکەوتووی زیاد کرا",
			...
			if (preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["EditName"])){
				...
				if ($answer == 'ok') {
					...
					echo '<script>
						...
						swal({
							type: "success",
							title: "گۆرانکاری لە بەکارهێنەرەکە کرا",
							showConfirmButton: true,
							confirmButtonText: "داخستن"
	...
	static public function ctrDeleteUser(){
		...
		if(isset($_GET["userId"])){
			...
			if($answer == "ok"){
				...
				swal({
					  type: "success",
					  title: "بەکارهێنەرەکە بە سەرکاوتووی سرایەوە",
					  showConfirmButton: true,
					  confirmButtonText: "Close"
